---
title: "Figure2c_Tconv7dafterBMT_inVivoDistribution"
...
knitr::write_bib(.packages(), "Figure2c_Tconv7dafterBMT_inVivoDistribution.packages.bib")
```
...
generating the plot for Figure 2c
...
p
pdf("./figures/TconvOrganDistSE.pdf", height=5.5, width=8.5)
p
dev.off()
...
shapiro.test(Tconv.dataLiver$count[1:7]) 						 # n.s.
shapiro.test(Tconv.dataLiver$count[8:13])						 # n.s.
shapiro.test(Tconv.dataLiver$count[14:17])                      # n.s.
```
...
shapiro.test(Tconv.dataColon$count[1:7]) 						 # n.s.
shapiro.test(Tconv.dataColon$count[8:13])						 # n.s.
shapiro.test(Tconv.dataColon$count[14:17])                      # n.s.
```
...
shapiro.test(Tconv.dataSpleen$count[1:7]) 						 # n.s.
shapiro.test(Tconv.dataSpleen$count[8:13])						 # n.s.
shapiro.test(Tconv.dataSpleen$count[14:17])                      # n.s.
```
...
writeLines(capture.output(sessionInfo(package = NULL)), "./Rscripts/Figure2c_Tconv7dafterBMT_inVivoDistribution_SessionInfo.txt")
```
