---
title: "FigureS2c_TconvPercent.7dafterBMT_inVivoDistribution"
...
knitr::write_bib(.packages(), "FigureS2c_TconvPercent.7dafterBMT_inVivoDistribution.packages.bib")
```
...
generating the plot for Supplementary Figure 2c
...
p <- p + labs(title="re-isolated Tconv (GvHD)",x="organ", y = "% of leukocytes")
...
```{r}
kruskal.test(WBCperc~category, data=Tconv.dataLiver)
```

```{r}
kwAllPairsDunnTest(WBCperc~category, data=Tconv.dataLiver, p.adjust.method = "bonferroni")
```

noTreg_Tx-allo_Tx p < 0.002 **
...
```{r}
kruskal.test(WBCperc~category, data=Tconv.dataSpleen)
```

```{r}
kwAllPairsDunnTest(WBCperc~category, data=Tconv.dataSpleen, p.adjust.method = "bonferroni")
```

noTreg_Tx-allo_Tx p < 0.0012 **
...
writeLines(capture.output(sessionInfo(package = NULL)), "./Rscripts/FigureS2c_TconvPercent.7dafterBMT_inVivoDistribution_SessionInfo.txt")
```
