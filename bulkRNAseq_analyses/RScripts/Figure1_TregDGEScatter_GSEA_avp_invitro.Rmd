...
* This is necessary in case the "bulkRNAseq_analyses_initial_setup.Rmd" has not been run yet.
...
```{r}
if (sum(colnames(counts.ivt)==metad.avp.ivt$sample_ID)==sum(avp.ivt.ind)) {
...
pdf(file="./FIGDIR/Figure1e_TregDGEScatter_avp.ivt_TregSignature.pdf", height=2.6, width=2.2)
avp.ivt.scat
dev.off()
```
...
## Barcode plot visualizations (Supplementary Figure 1 d)
...
text(-5,4,expression(italic(P)[adj.] *"<"* ~0.012), adj = c(0,.5))
```

* Barcodeplot is exported.  

```{r}
pdf(file="./FIGDIR/SupplementaryFigure1d_avp.ivt.Barcodeplot_HALL_HYPOXIA.pdf", height=3.2, width=6)
barcodeplot(avp.ivt.tr.res$table$logFC, 
            index=H.ind[["HALLMARK_HYPOXIA"]], 
            xlab = bquote(log[2]*"FC"),
            labels=c("up in poly","up in allo"), 
...
text(-5,3,expression(italic(P)[adj.] *"<"* ~0.012), adj = c(0,.5))
dev.off()
```
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure1d_avp.ivt.Barcodeplot_HALL_IL2_STAT5.pdf", height=3.2, width=6)
barcodeplot(avp.ivt.tr.res$table$logFC, 
            index=H.ind[["HALLMARK_IL2_STAT5_SIGNALING"]], 
            xlab = bquote(log[2]*"FC"),
            labels=c("up in poly","up in allo"), 
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure1d_avp.ivt.Barcodeplot_KEGG_GLYCONEO.pdf", height=3.2, width=6)
barcodeplot(avp.ivt.tr.res$table$logFC, 
            index=K.ind[["KEGG_GLYCOLYSIS_GLUCONEOGENESIS"]],
            xlab = bquote(log[2]*"FC"),
            labels=c("up in poly","up in allo"), 
