---
title: "Figure1e_TregDGEScatter_avp_invitro"
subtitle: "Scatter plot indicating genes differentially expressed *in invitro* expanded allo versus polyTreg (as measured by RNAseq, n=6 independent experiments each). Treg-signature genes defined by Aubert et al. 2020 are labeled."
author: "David Dittmar"
date: "`r Sys.Date()`"
bibliography: characterization-of-a-regulatory-t-cells-molecular-meta-signature-identifies-the-pro-enkephalin-gene-as-a-novel-marker-in-mice.bib
nocite: '@*'
output:
  html_document:
    df_print: paged
...
knitr::write_bib(.packages(), "Figure1e_TregDGEScatter_avp_invitro_packages.bib")
knitr::opts_knit$set(root.dir = normalizePath(".."))
```

```{r}
# Load required counts, metadata and annotaion RDS files
counts.m.rm<-readRDS(file.path(getwd(),"RCTDIR/counts.m.rm.2021-06-09.rds"))
metad.m.rm<-readRDS(file.path(getwd(),"METADIR/metad.m.rm.2021-06-09.rds"))
stid<-readRDS(file.path(getwd(),"METADIR/stid_2020-12-21.rds"))
```
...
* The additional EntrezID column is required for compatibility with the "goana" package.
```{r}
...
```{r}
colnames(counts.ivt)==metad.avp.ivt$sample_ID
sum(colnames(counts.ivt)==metad.avp.ivt$sample_ID) 
```
...
write.table(x = avp.ivt.tr.qstat$table,
            file = file.path(getwd(),"ANALDIR/Figure1e_TregDGE_avp_invitro_FC1.5.txt"),
            quote=FALSE, 
            sep = "\t",
            col.names = TRUE,
            row.names = FALSE)
...
write.table(rownames(avp.ivt.tr.qstat.scat.filt.UP),
            file = file.path(getwd(),"ANALDIR/Figure1e_TregDGE_avp_invitro.filt.UP.txt"), #evtl ".sig"
            col.names = "GeneID",
            quote=FALSE, 
            row.names = FALSE)
write.table(rownames(avp.ivt.tr.qstat.scat.filt.DN),
            file = file.path(getwd(),"ANALDIR/Figure1e_TregDGE_avp_invitro.filt.DN.txt"),
            col.names = "GeneID",
            quote=FALSE, 
            row.names = FALSE)
```
...
#### Scatter Plot with GOI highlighted
...
pdf(file=file.path(getwd(),"FIGDIR","Figure1e_TregDGEScatter_avp_invitro.pdf.pdf"), height=2.6, width=2.2)
avp.ivt.scat
dev.off()
```
...
writeLines(capture.output(sessionInfo(package = NULL)), "./Rscripts/Figure1e_TregDGEScatter_avp_invitro_SessionInfo.txt")
```
