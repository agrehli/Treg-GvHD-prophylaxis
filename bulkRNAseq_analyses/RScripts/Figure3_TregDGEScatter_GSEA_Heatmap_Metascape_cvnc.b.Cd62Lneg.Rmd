...
* For full functionality, the counts.bclsn, group.bclsn and genes.df variables are used to do so.
```{r}
...
gst.camera.cvnc.bn <- camera.DGEList(dgel.bclsn,index=H.ind.cvnc.bn,design=bclsn.design.dge,contrast=bclsn.con,inter.gene.cor=0.05)
gst.camera.cvnc.bn
```
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3g_bclsn.Heatmap_indivZscores_qstat.bclsn.treat.lfc1.5.pdf", height=6.5, width=4.5)
heatmap.2(data.matrix(bclsn.jt.filtered[,metad.bclsn$sample_ID]), 
          Rowv=as.dendrogram(bclsn.hr.filtered), 
          Colv=as.dendrogram(bclsn.hc.filtered), 
          col = clustercol, breaks = col_breaks, 
...
```{r}
bcls.ind<-grepl("wt",metad.m.rm$appl)&(grepl("S", metad.m.rm$org)|grepl("C", metad.m.rm$org)|grepl("L", metad.m.rm$org))
bcls.ind
```
...
```{r}
#Subsetting the respective counts object (counts.m.rm)
counts.bcls<-counts.m.rm[,bcls.ind]
ncol(counts.bcls)
...
```{r, echo= FALSE}
metad.bcls<-metad.m.rm[bcls.ind,]
...
metad.bcls
group.bcls<-factor(as.character(metad.bcls$grp), levels = #c("wt_C_neg","wt_L_neg","wt_L_pos","wt_S_neg","wt_S_pos") # this cannot be true, there is a mistake in the bar plot, trying to find it now
                     c("wt_C_neg","wt_L_neg","wt_S_neg","wt_L_pos","wt_S_pos"))
```
## Re-ordering Counts Data Frame Columns
* Here, read counts are re-ordered according to the metadata object.
```{r}
...
* Only genes fulfilling that criterion are kept and also normalization factors are calculated.
```{r}
keep.bcls <- filterByExpr(dgel.bcls) # I guess gruop would have been extracted from dgel.bcls, but it 
#keep.bcls <- rowSums(cpm(dgel.bcls)>1) >= 8 #the way we used to filter before the filterByExpr function #ususnoty we use 5 here
...
* This will be the data plotted in the bar plots.
```{r}
...
```{r}
bcls.log.rpkm.4bar <- data.frame(bcls.log.rpkm)
...
```{r}
pdf(file = "./FIGDIR/SupplementaryFigure3i_bcls.organ.sig.barplots.expression.shifted+4.pdf",
