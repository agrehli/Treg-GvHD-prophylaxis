...
-   First, required data objects (counts, metadata, short transcript ID) and R
    packages are loaded.
-   This is the starting point for most analyses conducted on bulk RNA-seq data
...
-   For the analysis, the data and metadata have to be prepared and properly
    annotated.

## Annotation Data Frame

-   An annotation data frame with 4 columns is constructed.
-   The additional EntrezID column is required for possible downstream pathway
    analyses or annotation operations.
...
-   In this analysis, we focus on CD62L^-^ tissue-resident Treg from colon,
    spleen and liver.
...
-   Here, the comprehensive counts data frame is reduced using the subsetting
    index.
...
-   The original order of samples is introduced for downstream re-ordering of
    the read count table.
-   An additional group variable assigning colon-re-isolated samples to one
    group and spleen- or liver-resident Treg to the other is introduced, as
    well.
-   This enables the comparison of lymphoid vs. non-lymphoid tissue-resident
    Treg (CD62L^-^) in the steady-stat, underlying this analysis.
-   Additionally, a group variable for coloring to distinguish spleen and liver
    is introduced.
...
-   In order to avoid wrongful labelling, sample_IDs are used to confirm that
    the counts and metadata objects are consistent.
...
-   For full functionality, the counts.bclsn, organ.bclsn and genes.df variables
    are used to do so.
...
-   The "filterByExpr()" function is used to determine which genes have
    sufficiently large counts to be retained in a statistical analysis.
-   Only genes fulfilling that criterion are kept and also normalization factors
    are calculated.
...
-   Data exploration revealed that PCR cycles are most suitable for batch
    correction if we compare colon with liver/spleen.
...
-   Batch-correction for prep-batch can be carried out as in logCPM computation
    above and is consequently introduced as blocking factor.
...
-   Hereafter, dispersion is estimated and the biological coefficient of
...
-   Fitting genewise glms (generalized linear model) for the constructed
    designs.
...
-   The interest in this analysis lies in the differences between Treg
    re-isolated from colon versus liver & spleen.
-   Thus, our contrast of interest is "colon - liver_spleen" for the DGE
    analysis.
-   We can have a quick look at how many genes are DE with a FC \> 1.5 and FDR
    \< 0.05.
...
-   The data frame contained in the glmTreat function results is stored for
    direct access.

```{r}
write.table(x = bclsn.cvnc.tr.qstat$table,
            file = "./ANALDIR/SupplementaryFigure3g_TregDGE_cvnc_bas.neg_FC1.5.txt",
            quote=FALSE, 
            sep = "\t",
            col.names = TRUE,
            row.names = FALSE)
...
-   Alternative Design: We do all comparisons to combine differential gene
    lists.
...
-   The colon vs. non-colon resident CD62L^-^ Treg comparison will be the basis
    for this.
-   At the same time the comparison can be understood as non-lymphoid tissue
    (NLT) vs. lymphoid tissue (LT), in line with the nomenclature of the
    Miragaia et al. 2019 paper.

## Data Preparation

-   In order not to exclude GOI, the data is no further filtered than already
    facilitated above when accounting for rare transcripts.
-   Necessary data transformations for the scatter plot are carried out as well:
    columns with individual logCPMs (calculated from logCPM and logFC).
...
```{r}
bclsn.cvnc.tr.qstat.scat.UP%>% head
```
...
write.table(rownames(bclsn.cvnc.tr.qstat.scat.UP),
            file = "./ANALDIR/SupplementaryFigure3g_TregDGE_cvnc_bas.neg.sig.UP.txt",
            col.names = "GeneID",
            quote=FALSE, 
            row.names = FALSE)
write.table(rownames(bclsn.cvnc.tr.qstat.scat.DN),
            file = "./ANALDIR/SupplementaryFigure3g_TregDGE_cvnc_bas.neg.sig.DN.txt",
            col.names = "GeneID",
            quote=FALSE, 
            row.names = FALSE)
```
...
-   Hereafter, GOI are taken from Miragaia et al. 2019 (Figure 2 B) presenting
    Treg subpopulations of the steady-state.
-   The question addressed here is whether our data is in line with their
    findings.
-   [Here you can have a look at the
    figure](https://pubmed.ncbi.nlm.nih.gov/30737144/#&gid=article-figures&pid=figure-2-uid-2)
...
-   First, we prepare some labels to be able to label data points for all
    categories with a single call to geom_text_repel.
...
### Scatter plot with GOI highlighted (Supplementary Figure 3 g)
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3g_TregDGEScatter_cvnc.bn_tmGenes.pdf",
...
-   Using camera, we look whether pathways from hallmark gene sets are enriched
    in our regulated genes
...
```{r, data export}
write.table (as.data.frame(filter(gst.hall.cvnc.bn.ord,FDR < 0.05)) %>% tibble::rownames_to_column("Hallmark"), file = "./ANALDIR/dataFigureS3h.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```

## GSEA Barcode Plots (Supplementary Figure 3 h)
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3h_Barcodeplot_cvnc.bas.neg_Hallmark_TNFA.pdf", height=3.2, width=6)
bclsn.cvnc.bc.tnfa
dev.off()
```
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3h_Barcodeplot_cvnc.bas.neg_Hallmark_IL6JS.pdf", height=3.2, width=6)
bclsn.cvnc.bc.il6js
dev.off()
```
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3h_Barcodeplot_cvnc.bas.neg_Hallmark_IL2STAT5.pdf", height=3.2, width=6)
bclsn.cvnc.bc.il2s5
dev.off()
```
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3h_Barcodeplot_cvnc.bas.neg_Hallmark_IR.pdf", height=3.2, width=6)
bclsn.cvnc.bc.infr
dev.off()
```

-   Hereafter, the top 4 enriched pathways will be plotted as barcodeplots, FDR
    ranked.
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3h_Barcodeplot_cvnc.bas.neg_Hallmark_top4ranked.pdf", height = 3.2, width=6)
# for loop for top 4 plotting
for (i in c(1,2,3,4)) {
  #message(i)
    bc.tmp %<a-% {
...
-   Hereafter, the heat map shown in Supplementary Figure 3 i will be generated.

## Required Data Frames

-   The scaled and prep-batch-corrected data is the data for plotting.
-   DGE test statistics are extracted and defined as variables to be extended by
    logRPKM based on logCPM and gene length.
...
-   The filtering strategy combines interesting DEGs for the colon vs. non-colon
    as well as individual pair wise comparisons.
...
-   Also, colors for the cluster sidebar are defined with the 2 colon-specific
    clusters combined.
...
-   Gene symbols for clusters 1,2,3 and 4 (4 and 5 merged) are extracted in the
    order of appearance in above heat map from top to bottom.
-   Respective background genes resulting from a filtering only by logCPM and
    logRPKM are defined, as well.
-   Additionally, GeneIDs from colon down and up genes are extracted for gene
    set rotation analyses of respective comparison results in another sample
    subset.
...
```{r, counts}
write.table (as.data.frame(bclsn.jt.filtered.clusterID.ordered) %>% tibble::rownames_to_column("ShortID"), file = "./ANALDIR/dataFigureS3i.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```

## Basic Heat Map (Supplementary Figure 3 i)

-   Following filtering for DEGOI and hierarchical clustering of genes and
    samples, data is presented as heat map.
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3i_bclsn.Heatmap_indivZscores_qstat.bclsn.treat.lfc.0.6.pdf", height=8.5, width=6.5)
bclsn.cvnc.hm.pryr.ann
dev.off()
```
...
-   Cluster and background gene lists are exported for a custom metascape
    analysis.
-   For clusters 1 and 4 (colon up and colon down), GeneIDs are exported
    additionally for GSEA barcode plots in another analysis.

```{r}
# clusters from hea tmap
write.table(cluster1.genes,
            file = "./ANALDIR/SupplementaryFigure3i_cluster1.colon.down.txt",
            col.names = "GeneSymbol",
            quote=FALSE, 
            row.names = FALSE)
write.table(cluster2.genes,
            file = "./ANALDIR/SupplementaryFigure3i_cluster2.liver.up.txt",
            col.names = "GeneSymbol",
            quote=FALSE, 
            row.names = FALSE)
write.table(cluster3.genes,
            file = "./ANALDIR/SupplementaryFigure3i_cluster3.spleen.up.txt",
            col.names = "GeneSymbol",
            quote=FALSE, 
            row.names = FALSE)
write.table(cluster4.genes,
            file = "./ANALDIR/SupplementaryFigure3i_cluster4.colon.up.txt",
            col.names = "GeneSymbol",
            quote=FALSE, 
            row.names = FALSE)
write.table(bclsn.bg.genes,
            file = "./ANALDIR/SupplementaryFigure3i_filtered.background.genes.txt",
            col.names = "GeneSymbol",
            quote=FALSE, 
            row.names = FALSE)
...
-   A manual metascape analysis is run with the clusters gene lists customized
    background gene list.
...
```{r, fig.width=3.6}
bclsn.metascape.down <- read_excel("./METASCAPE/SupplementaryFigure3i_cluster1.colon.down_metascape_result.xlsx", sheet=2)
...
```{r, fig.width=3.6}
bclsn.metascape.up <- read_excel("./METASCAPE/SupplementaryFigure3i_cluster4.colon.up_metascape_result.xlsx", sheet=2)
...
### Pathway analysis (Metascape) (Supplementary Figure 3 j)

-   Hereafter, significantly enriched pathways as determined by the custom
    metascape analysis are presented as bar plots.
-   Only for heat map clusters (Supplementary Figure 3 i) 1 and 4, pathways were
    found enriched significantly.
...
```{r}
pdf(file="./FIGDIR/SupplementaryFigure3j_bclsn.Metascape.Barplot.pdf", height=5.6, width = 4.6)
bclsn.metascape.plot.grid
dev.off()
```
...
-   For the bar plot in Supplementary Figure 3 k, we also need to include
    CD62L^+^ samples.
...
-   A DGEList object is generated, the main class of the edgeR package.
-   For full functionality, the counts.bcls, group.bcls and genes.df variables
    are used to do so.
...
-   log2-transformed RPKM bar plots for selected genes are produced by the
    following code.
-   Before bar plots can be generated, an appropriate data frame has to be
    created.
...
-   Values are converted to numeric and the GeneSymbol column is declared a
    factor to enable desirable ordering.
...
```{r, rpkm_table}
write.table (as.data.frame(bcls.log.rpkm.4bar.organ.dm), file = "./ANALDIR/dataFigureS3k.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```

## Bar plot arrangement (Supplementary Figure 3 k)

-   Bar plots are created and arranged into an array of plots.
-   Scale labeling is adjusted to match the manual data transformation for more
    aesthetic plotting.
...
```{r}
pdf(file = "./FIGDIR/SupplementaryFigure3k_bcls.organ.sig.barplots.expression.shifted+4.prepbatch-corrected.pdf",
