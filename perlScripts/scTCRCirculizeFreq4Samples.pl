...
if (@ARGV < 2) {
	print STDERR "\nscTCRCirculizeFreq4Samples.pl <tableFile> -outDir <outputDir> -name <output file name>\\\n";
...
while (<IN>) {
	...
	if ($. == 1) {
		...
		$col = $i + 1;
		$sums[$i] =  `awk '{s+=\$'$col'} END {print s}' $ARGV[0]`;
...
open SCRIPT, ">$rScript";
	...
	print SCRIPT "colnames(table) <- c(\"Cell type\",\"Cell number\")\n";
