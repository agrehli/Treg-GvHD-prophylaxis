---
title: "FigureS6cS6d_Treg_GvHD_TRBdiversity-singleClones_10x_TRB"
...
knitr::write_bib(.packages(), "FigureS6cS6d_Treg_GvHD_TRBdiversity-singleClones_10x_TRB.packages.bib")
```

This Rscript is called in the Commands_scTCR_GvHD.sh script

# Data analysis for Figures S6c and S6d 
...
```{r}
write.table (as.data.frame(div_simp), file = "./figures/scTCRxTregDCLS_diversity_invSimp_3.4K_sourceData.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```


generating the plot for Figure S6c (will remove the error bar in the donor samples, just two samples!)
...
running statistical testing with triplicate tissue samples only
Anova with Tukey

(adding corresponding bars and asterisks to the plot in Adobe Illustrator)

```{r}
stat.data <- div_simp[3:11,]
res.aov <- aov(Value~Type, data=stat.data)
summary (res.aov)
```
...
```
                ...
                p adj
liver-colon   0.0047997
spleen-colon  0.0116720
spleen-liver  0.6720884





## alternative visualization of repertoire overlap (Figure S6d)
...
exporting data table
```{r}
write.table (as.data.frame(imm_ov2), file = "./figures/scTCRxTregDCLS_overlap_morista_heatmap_sourceData.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
writeLines(capture.output(sessionInfo(package = NULL)), "FigureS6cS6d_Treg_GvHD_TRBdiversity-singleClones_10x_TRB_SessionInfo.txt")
```
